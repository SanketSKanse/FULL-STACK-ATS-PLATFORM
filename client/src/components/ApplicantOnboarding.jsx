...
export default function ApplicantOnboarding({ user, initialProfile, onComplete }) {
  ...
  const save = async () => {
    ...
    try {
      const response = await axios.put('http://localhost:5001/api/applicant/profile', { ...profile, profileCompleted: true }, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
...
function Field({ label, value, onChange, placeholder, disabled, required, error, onBlur, type = 'text', inputMode, maxLength }) { return <label className="block"><span className="mb-1.5 block text-sm font-medium text-slate-700">{label}{required && <span className="text-red-500"> *</span>}</span><input type={type} inputMode={inputMode} maxLength={maxLength} value={value || ''} onChange={(event) => onChange?.(event.target.value)} onBlur={onBlur} placeholder={placeholder} disabled={disabled} className={`w-full rounded-xl border bg-slate-50 px-3.5 py-2.5 text-sm outline-none focus:bg-white disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-500 ${error ? 'border-red-300 focus:border-red-400' : 'border-slate-200 focus:border-indigo-300'}`} />{error && <span className="mt-1 block text-xs text-red-600">{error}</span>}</label>; }
...
function ChoiceGroup({ label, field, values, profile, toggle }) { return <div><div className="mb-2 text-sm font-medium text-slate-700">{label}</div><div className="flex flex-wrap gap-2">{values.map((value) => <button key={value} type="button" aria-pressed={profile[field].includes(value)} onClick={() => toggle(field, value)} className={`rounded-full border px-3 py-2 text-sm transition ${profile[field].includes(value) ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-600 hover:border-indigo-200'}`}>{value}</button>)}</div></div>; }
function TagInput({ label, field, profile, addText, placeholder }) { const [value, setValue] = useState(''); return <div><label className="mb-1.5 block text-sm font-medium text-slate-700">{label}</label><div className="flex gap-2"><input value={value} onChange={(event) => setValue(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); addText(field, value); setValue(''); } }} placeholder={placeholder} className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm outline-none focus:border-indigo-300 focus:bg-white" /><button type="button" onClick={() => { addText(field, value); setValue(''); }} className="inline-flex items-center gap-1 rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-600"><Plus size={15} /> Add</button></div><div className="mt-2 flex flex-wrap gap-2">{profile[field].map((item) => <span key={item} className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-medium text-slate-600">{item}</span>)}</div></div>; }
function RepeatableSection({ title, items, field, blank, update, renderItem }) { return <div><div className="mb-4 flex items-center justify-between"><div><h2 className="text-lg font-semibold text-slate-900">{title}</h2><p className="mt-1 text-sm text-slate-500">Add as many entries as apply to you.</p></div><button type="button" onClick={() => update(field, [...items, { ...blank }])} className="inline-flex items-center gap-1 rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-600"><Plus size={15} /> Add {title.toLowerCase()}</button></div>{items.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-10 text-center text-sm text-slate-500">No {title.toLowerCase()} added yet. You can continue without one.</div> : <div className="space-y-4">{items.map((item, index) => <div key={item._id || index} className="rounded-2xl border border-slate-200 p-4"><div className="mb-4 flex justify-end"><button type="button" onClick={() => update(field, items.filter((_, itemIndex) => itemIndex !== index))} aria-label={`Remove ${title.toLowerCase()}`} className="text-slate-400 hover:text-red-600"><Trash2 size={16} /></button></div>{renderItem(item, index)}</div>)}</div>}</div>; }